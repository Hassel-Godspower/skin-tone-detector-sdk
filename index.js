...
export function getClosestTone(hex) {
  ...
  for (const [toneName, toneHex] of Object.entries(predefinedSkinTones)) {
  ...
  const bestHex = predefinedSkinTones[bestMatch];

  return {
    tone: bestMatch,
    hex: bestHex,
    deltaE: parseFloat(lowestDelta.toFixed(3)),
    rgb: hexToRgb(bestHex),
    message: `Detected tone: ${bestMatch} (${bestHex}) with ΔE=${lowestDelta.toFixed(
      2
    )}`,
  };
}
...
// ================= Public API =================
export function analyzeSkinColor({ r, g, b }) {
  const hex = rgbToHex(r, g, b);
  return getClosestTone(hex);
}
